      ...
      {pending && typeof window !== "undefined" &&
        createPortal(
          (() => {
            ...
            // 모바일: 시스템 selection callout 은 여백이 큰 쪽으로 떠서
            //   - 상반부 선택 → 시스템 아래 → 우리 pill 은 위로
            //   - 하반부 선택 → 시스템 위 → 우리 pill 은 아래로
            // 데스크톱: 시스템 callout 이 없으므로 위쪽 우선, 공간 부족 시에만 아래로
            const selCenterY = (pending.rect.top + pending.rect.bottom) / 2;
            const inUpperHalf = selCenterY < window.innerHeight / 2;
            const flipBelow = isTouch
              ? !inUpperHalf
              : pending.rect.top < BTN_H + GAP + 4;
            const top = flipBelow
