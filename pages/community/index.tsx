...
function PostCard({ post }: { post: Post }) {
  const preview = post.content.slice(0, 160).replace(/\n+/g, " ").replace(/[<>\[\]]/g, "").trim();
  ...
  return (
    <Link href={`/community/${post.id}`} className="block group">
      <div className="bg-white rounded-2xl border border-gray-200/60 p-5 hover:shadow-md hover:border-navy-200/60 transition-all h-full flex flex-col shadow-sm">
        <div className="flex items-start justify-between gap-2 mb-3">
          <span className="text-xs font-mono bg-navy-50 text-navy-700 px-2.5 py-1 rounded-full border border-navy-100/80">
            {post.caseNumber}
          </span>
          <span className="text-xs text-gray-300 whitespace-nowrap">{date}</span>
        </div>

        <h3 className="font-serif font-semibold text-gray-900 text-sm mb-1 group-hover:text-navy-700 transition-colors line-clamp-2 leading-snug">
          {post.caseName || post.caseNumber}
        </h3>
        {post.court && (
          <p className="text-xs text-gray-400 mb-2">{post.court}</p>
        )}
        <p className="text-xs text-gray-400 leading-relaxed flex-1 line-clamp-3 mt-1">{preview}</p>
    ...
    <Layout title="커뮤니티 - 변시 민사법 사례 생성기">
      <div className="max-w-6xl mx-auto px-4 py-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <h1 className="text-2xl sm:text-3xl font-serif font-bold text-gray-900 mb-1">커뮤니티</h1>
            <p className="text-sm text-gray-400">수험생들이 공유한 사례형 문제를 열람하고 함께 학습하세요.</p>
          </div>
          <div className="flex items-center gap-2.5">
            <div className="flex gap-0.5 bg-gray-100 rounded-xl p-1">
              {(["createdAt", "likes"] as const).map((s) => (
                <button
                  key={s}
                  onClick={() => setSortBy(s)}
            ...
            <Link href="/generate" className="btn-gold text-sm">
        ...
        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-white rounded-2xl border border-gray-200/60 p-5 animate-pulse shadow-sm">
          ...
          <div className="text-center py-28">
            ...
            <h3 className="font-serif font-semibold text-gray-900 text-lg mb-2">아직 게시물이 없습니다</h3>
            <p className="text-gray-400 text-sm mb-6">첫 번째로 사례형 문제를 생성하고 공유해 보세요!</p>
            <Link href="/generate" className="btn-gold text-sm">문제 생성하기</Link>
          </div>
        ) : (
          <>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
